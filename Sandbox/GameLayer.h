...
public:
  GameLayer(std::string name)
      : Layer(name), m_CameraController(Gnote::OrthographicCameraController(
                         1280 / 720, true, true)),
        m_PerspectiveCamera(Gnote::PerspectiveCamera(
            glm::vec3(0.0f, 0.0f, -10.0f), 1280 / 720)) {}

  virtual void OnAttach() override {

    Gnote::Ref<Gnote::Shader> m_Shader = m_ShaderLibarry.Load(
        "Shader", "./build/bin/Assets/Shader/VertexShader.glsl",
        "./build/bin/Assets/Shader/FragmentShader.glsl");
    ...
    Gnote::BufferLayout cubeLayout(
        {{Gnote::ShaderDataType::Float3, "aPos"},
         {Gnote::ShaderDataType::Float2, "aTextureNormal"}});
  ...
  virtual void OnEvent(Gnote::Event &event) override {
    ...
    dispatcher.Dispatcher<Gnote::MouseButtonPressed>(
        GNOTE_EVENT_BIND(GameLayer::OnMousePressed));
    m_CameraController.OnEvent(event);
  }

  bool OnMousePressed(Gnote::MouseButtonPressed &event) { return false; }
