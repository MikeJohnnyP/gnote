...
class Layer2D : public Gnote::Layer
{
  public:
    Layer2D(std::string name)
        : Layer(name), m_CameraController(Gnote::OrthographicCameraController((float)1280 / 720, true, true))
    {
    }
    ~Layer2D()
    {
    ...
    {
        m_Texture = Gnote::Texture2D::Create("./build/bin/Assets/awesomeface.png");
    }
    ...
    virtual void OnUpdate(Gnote::TimeSteps &ts) override
    {
        ...
        for (auto i = 0; i < 10; i++)
        {
            for (auto y = 0; y < 10; y++)
            {
                Gnote::Renderer2D::DrawQuad(glm::vec2(0.11f * i, 0.11f * y), glm::vec2(0.1f),
        ...
        for (auto i = 0; i < 10; i++)
        {
            for (auto y = 0; y < 10; y++)
            {
                Gnote::Renderer2D::DrawQuad(glm::vec2(0.22f * i, 0.11f * y), glm::vec2(0.1f), m_Texture);
            }
        }
