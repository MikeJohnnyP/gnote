...
public:
  static DllManager *getInstance() { return m_instance; }
  ...
  template <typename T>
  static T GetStructModuleAPI(const std::string &moduleName,
                              const std::string &createFuncName) {
    ...
    if (it != instance->loadedDlls.end()) {
      ...
      if (func) {
        ...
        GNOTE_CORE_ASSERT(false, "Failed to load function: {0}",
                          createFuncName);
      }
    }
    return nullptr;
