...
namespace Gnote {
KeyInputPlatform *createKeyInput() { return new WindowsKeyboardInput(); }
MouseInputPlatform *createMouseInput() { return new WindowMouseInput(); }
...
bool WindowsWindow::Init() {
  ...
  glfwSetWindowSizeCallback(m_GLFWwindow, [](GLFWwindow *window, int width,
                                             int height) {
  ...
  glfwSetKeyCallback(m_GLFWwindow, [](GLFWwindow *window, int key, int scancode,
                                      int action, int mods) {
    WindowData &data = *(WindowData *)glfwGetWindowUserPointer(window);
    switch (action) {
    case GLFW_PRESS: {
  ...
  glfwSetScrollCallback(
      m_GLFWwindow, [](GLFWwindow *window, double xoffset, double yoffset) {
        WindowData &data = *(WindowData *)glfwGetWindowUserPointer(window);
  ...
  glfwSetCharCallback(
      m_GLFWwindow, [](GLFWwindow *window, unsigned int codepoint) {
        WindowData &data = *(WindowData *)glfwGetWindowUserPointer(window);
...
void WindowsWindow::Shutdown() { glfwDestroyWindow(m_GLFWwindow); }
void WindowsWindow::SwapBuffer() { m_context->SwapBuffer(); }
void WindowsWindow::PollEvent() { glfwPollEvents(); }
bool WindowsWindow::IsWindowClose() {
  return glfwWindowShouldClose(m_GLFWwindow);
}
bool WindowsWindow::IsVsyncEnable() { return m_data.vSync; }
void WindowsWindow::SetFunctionCallback(const EventFn &callback) {
  m_data.callback = callback;
}

void *WindowsWindow::GetNativeWindow() { return (void *)m_GLFWwindow; }

InputState *WindowsWindow::GetInputState() { return &m_data.inputState; }

float WindowsWindow::GetTime() { return (float)glfwGetTime(); }
