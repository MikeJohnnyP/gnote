...
namespace Gnote
{
PerspectiveCamera::PerspectiveCamera(const glm::vec3 &cameraPos, float aspectRatio)
    : m_Fov(45.f), m_AspectRatio(aspectRatio), m_CameraOrientation(glm::vec3(0.0f, 0.0f, 0.0f)), m_CameraPos(cameraPos),
      m_View(glm::mat4(1.0f)), m_Projection(glm::perspective(glm::radians(m_Fov), aspectRatio, 0.1f, 1000.f)),
...
const float &PerspectiveCamera::GetRotation() const
{
    return 0.f;
}
